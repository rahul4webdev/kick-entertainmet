...
class SettingsScreenController extends BaseController {
  ...
  onChangedToggle(bool value, SettingToggle settingToggle) async {
    ...
    await UserService.instance.updateUserDetails(
        ...
        isPrivate:
            settingToggle == SettingToggle.privateAccount ? value : null,
        hideOthersLikeCount:
            settingToggle == SettingToggle.hideOthersLikeCount ? value : null,
        commentApprovalEnabled:
            settingToggle == SettingToggle.commentApproval ? value : null);
    isUpdateApiCalled.value = false;
  ...
  void onResetFeed() {
    Get.bottomSheet(ConfirmationSheet(
        onTap: () async {
          ...
          StatusModel model = await UserService.instance.resetFeed();
          stopLoader();
          if (model.status == true) {
            showSnackBar(LKey.feedResetSuccess.tr);
          } else {
            showSnackBar(model.message);
          }
        },
        description: LKey.resetFeedConfirm.tr,
        title: LKey.resetFeed.tr));
  }
  ...
  Future<void> deleteCurrentUser() async {
    // Account deletion is handled server-side via deleteMyAccount API.
    // No Firebase Auth cleanup needed — custom auth uses backend tokens only.
    Loggers.success("User account deleted via backend API.");
  }
      ...
      onTap: () async {
        ...
        try {
          ...
          if (result.status == true) {
            ...
            SessionManager.instance.removeAccountSession(currentId);
