...
import 'package:shortzz/common/service/api/location_review_service.dart';
...
class LocationScreenController extends BaseController {
  ...
  RxList<LocationReview> reviews = <LocationReview>[].obs;
  ...
  Future<void> initData() async {
    ...
    List<Post> reels = result[0] as List<Post>;
    List<Post> posts = result[1] as List<Post>;
  ...
  Future<List<LocationReview>> fetchReviews({bool reset = false}) async {
    isReviewLoading.value = true;
    try {
      final lastItemId = reset || reviews.isEmpty
          ? null
          : reviews.map((r) => r.id ?? 0).reduce((a, b) => a < b ? a : b);
      final response = await LocationReviewService.instance.fetchLocationReviews(
        placeTitle: placeTitle.value,
        placeLat: latLng.value.latitude,
        placeLon: latLng.value.longitude,
        lastItemId: lastItemId,
      );
  ...
  Future<bool> submitReview({required int rating, String? reviewText}) async {
    try {
      final response = await LocationReviewService.instance.submitReview(
        placeTitle: placeTitle.value,
        placeLat: latLng.value.latitude,
        placeLon: latLng.value.longitude,
        rating: rating,
  ...
  Future<bool> deleteReview(int reviewId) async {
    try {
      final response = await LocationReviewService.instance.deleteReview(reviewId: reviewId);
      if (response.status == true) {
        reviews.removeWhere((r) => r.id == reviewId);
        reviewCount.value = (reviewCount.value - 1).clamp(0, 999999);
        return true;
      }
    } catch (_) {}
    return false;
