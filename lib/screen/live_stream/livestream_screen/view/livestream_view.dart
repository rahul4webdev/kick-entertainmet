  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      return switch (coHostCount) {
        ...
        _ when coHostCount >= 5 => MultiGuestGridView(
...
/// Grid layout for 5-8 guests: top row has host large, remaining in 2-column grid below
class MultiGuestGridView extends StatelessWidget {
  final LivestreamScreenController controller;
  final List<StreamView> streamViews;

  const MultiGuestGridView({
    super.key,
    required this.controller,
    required this.streamViews,
  });
  ...
  @override
  Widget build(BuildContext context) {
    // Host gets a larger slot on top, remaining guests in 2-column grid
    final host = streamViews.first;
    final guests = streamViews.sublist(1);
    final rows = <List<StreamView>>[];
    for (int i = 0; i < guests.length; i += 2) {
      rows.add(guests.sublist(i, (i + 2).clamp(0, guests.length)));
    }

    return Column(
      children: [
        // Host view — takes 40% of height
        Expanded(
          flex: 4,
          child: LiveStreamUserView(
            isNameAndSpeakerVisible: false,
        ...
        // Guest rows — each row takes equal space
        ...rows.map((row) => Expanded(
              flex: 3,
              child: Row(
                children: [
                  for (final guest in row)
                    Expanded(
                      child: LiveStreamUserView(
                        controller: controller,
                        streamingView: guest,
