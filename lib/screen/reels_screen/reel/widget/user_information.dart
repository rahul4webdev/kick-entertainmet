  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          StitchBadge(controller: controller),
          DuetBadge(controller: controller),
          AiGeneratedBadge(controller: controller),
...
class DuetBadge extends StatelessWidget {
  final ReelController controller;

  const DuetBadge({super.key, required this.controller});

  @override
  Widget build(BuildContext context) {
    final post = controller.reelData.value;
    if (!post.isDuet) return const SizedBox.shrink();

    final sourceUser = post.duetSource?.user;
    final sourceUsername = sourceUser?.username ?? 'unknown';
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () {
          if (sourceUser != null) {
            NavigationService.shared.openProfileScreen(sourceUser);
          }
        },
        child: Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Image.asset(AssetRes.icDuet,
                  width: 14, height: 14, color: whitePure(context)),
              const SizedBox(width: 4),
              Text(
                'Duet with @$sourceUsername',
...
class StitchBadge extends StatelessWidget {
  ...
  const StitchBadge({super.key, required this.controller});

  @override
  Widget build(BuildContext context) {
    final post = controller.reelData.value;
    if (!post.isStitch) return const SizedBox.shrink();

    final sourceUser = post.stitchSource?.user;
    final sourceUsername = sourceUser?.username ?? 'unknown';
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Image.asset(AssetRes.icStitch,
                  width: 14, height: 14, color: whitePure(context)),
              const SizedBox(width: 4),
              Text(
                'Stitch with @$sourceUsername',
                style: TextStyleCustom.outFitMedium500(
                    fontSize: 11, color: whitePure(context)),
              ),
            ],
...
class AiGeneratedBadge extends StatelessWidget {
  ...
  const AiGeneratedBadge({super.key, required this.controller});

  @override
  Widget build(BuildContext context) {
    final post = controller.reelData.value;
    if (!post.isAiGenerated) return const SizedBox.shrink();
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.smart_toy, size: 14, color: Colors.amber.shade600),
            const SizedBox(width: 4),
            Text(
              LKey.aiGenerated.tr,
              style: TextStyleCustom.outFitMedium500(
                  fontSize: 11, color: Colors.amber.shade600),
            ),
          ],
        ),
      ),
      ...
      return InkWell(
        ...
        child: isLoading.value
            ...
            : Container(
                ...
                child: Text(
                  isFollow ? LKey.following.tr : LKey.follow.tr,
