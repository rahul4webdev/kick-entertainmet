...
class CommentSheetController extends BaseController {
  ...
  // Sort mode: 'newest' or 'top'
  RxString sortMode = 'newest'.obs;
  RxList<Comment> topCommentsList = <Comment>[].obs;
  RxBool isLoadingTopComments = false.obs;

  bool get isPostOwner =>
      post.value?.userId == SessionManager.instance.getUserID();
  ...
  Future<void> fetchComments({bool isEmpty = false}) async {
    ...
    try {
      ...
      // Track pending count for post owner
      if (isPostOwner && items.pendingCount != null) {
        pendingCount.value = items.pendingCount!;
      }
    } catch (e) {
          ...
          onUpdateComment: (comment, isReplyComment) {
            if (isReplyComment) {
              ...
              if (comment.isApproved == false) {
                showSnackBar(LKey.commentPendingApproval.tr);
              } else {
  ...
  RxList<Comment> get getActiveCommentsList {
  ...
  Future<void> fetchTopComments({bool isEmpty = false}) async {
    final postId = post.value?.id?.toInt() ?? -1;
    if (postId == -1) return;
    if (isLoadingTopComments.value) return;
    isLoadingTopComments.value = true;

    try {
      final items = await PostService.instance.fetchTopComments(
        postId: postId,
        lastItemId: isEmpty ? null : topCommentsList.lastOrNull?.id?.toInt(),
      );
      ...
      for (final c in items) {
        if (!topCommentsList.any((existing) => existing.id == c.id)) {
          topCommentsList.add(c);
        }
      }
    } catch (e) {
      Loggers.error('Error fetching top comments: $e');
    } finally {
      isLoadingTopComments.value = false;
    }
  }

  // Creator like/unlike
  Future<void> creatorLikeComment(Comment? comment) async {
    if (comment == null) return;
    int commentId = comment.id?.toInt() ?? -1;
    ...
    _refreshCommentInList(comment);

    StatusModel response = await PostService.instance
        .creatorLikeComment(commentId: commentId);
    if (response.status == false) {
      comment.isCreatorLiked = false;
      _refreshCommentInList(comment);
    }
  }

  Future<void> creatorUnlikeComment(Comment? comment) async {
    if (comment == null) return;
    int commentId = comment.id?.toInt() ?? -1;
    if (commentId == -1) return;
    ...
    StatusModel response = await PostService.instance
        .creatorUnlikeComment(commentId: commentId);
    if (response.status == false) {
      comment.isCreatorLiked = true;
      _refreshCommentInList(comment);
    }
  ...
  void _refreshCommentInList(Comment comment) {
    int idx = commentsList.indexWhere((e) => e.id == comment.id);
    if (idx != -1) {
      commentsList[idx] = comment;
    }
    int topIdx = topCommentsList.indexWhere((e) => e.id == comment.id);
    if (topIdx != -1) {
      topCommentsList[topIdx] = comment;
    }
  }
}
