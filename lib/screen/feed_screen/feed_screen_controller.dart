...
class FeedScreenController extends BaseController {
  ...
  Rx<PostCategory> selectedPostCategory = PostCategory.trending.obs;
  ...
  initData() {
    Future.wait([_fetchTrendingPosts(), _fetchStory()]);
  }
  ...
  Future<void> _fetchTrendingPosts({bool isEmpty = false}) async {
    if (isLoading.value) return;
    isLoading.value = true;
    PostsModel model =
        await PostService.instance.fetchTrendingPosts(type: PostType.posts);
    _addDataInPostList(model.data ?? [], model.adPositions ?? [], isEmpty);
  }
  ...
  _addDataInPostList(List<Post> newList, List<int> adPositions, bool isEmpty) async {
    if (isEmpty) {
      posts.clear();
      feedItems.clear();
    }
    posts.addAll(newList);
    feedItems.addAll(FeedItemMerger.merge(newList, adPositions));
  ...
  Future<void> onChangeCategory(PostCategory value) async {
    ...
    switch (value) {
      case PostCategory.trending:
        await _fetchTrendingPosts(isEmpty: true);
