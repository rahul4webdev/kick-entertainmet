...
class ChatBottomActionView extends StatelessWidget {
  ...
  void _showSchedulePicker(BuildContext context) async {
    ...
    if (scheduled.isBefore(DateTime.now())) {
      controller.showSnackBar('Please select a future time');
      return;
    }

    controller.onScheduleTextMessage(scheduled);
  }
              ...
              return Column(
                ...
                children: [
                  if (controller.replyingToMessage.value != null)
                    _ReplyIndicator(controller: controller),
                  if (controller.isEditing)
                    _EditingIndicator(controller: controller),
                  Stack(alignment: Alignment.center, children: [
                    ChatTextField(
                        ...
                        onScheduleTap: () => _showSchedulePicker(context),
                        actions: ChatAction.getChatActions(
                            isGiphyEnabled:
                                controller.setting?.gifSupport == 1)),
                          ...
                          GestureDetector(
                            ...
                            child: Padding(
                              ...
                              child: Icon(Icons.schedule_send,
                                  size: 22, color: textLightGrey(context)),
                            ),
                          ),
                        InkWell(
...
class _ReplyIndicator extends StatelessWidget {
  ...
  const _ReplyIndicator({required this.controller});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.reply, size: 16, color: themeAccentSolid(context)),
...
class _EditingIndicator extends StatelessWidget {
  ...
  const _EditingIndicator({required this.controller});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.edit, size: 16, color: themeAccentSolid(context)),
          const SizedBox(width: 6),
          Expanded(
            child: Text(
              LKey.editingMessage.tr,
              style: TextStyleCustom.outFitRegular400(
          ...
          GestureDetector(
            onTap: controller.cancelEditMessage,
            child: Icon(Icons.close, size: 18, color: textLightGrey(context)),
          ),
        ],
      ),
