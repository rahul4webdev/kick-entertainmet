...
class RequestWithdrawalScreenController extends BaseController {
  ...
  // Bank accounts
  RxList<BankAccount> savedAccounts = <BankAccount>[].obs;
  Rx<BankAccount?> selectedSavedAccount = Rx<BankAccount?>(null);
  RxBool useSavedAccount = false.obs;
  ...
  Future<void> fetchSavedAccounts() async {
    savedAccounts.value =
        await BankAccountService.instance.fetchBankAccounts();
    // Auto-select default account if available
    final defaultAccount =
        savedAccounts.where((a) => a.isDefault).firstOrNull;
    if (defaultAccount != null) {
      selectSavedAccount(defaultAccount);
    }
  }

  void selectSavedAccount(BankAccount account) {
    selectedSavedAccount.value = account;
    useSavedAccount.value = true;
    selectedGateway.value = account.gateway ?? '';
    accountDetailsController.text = account.accountDetails ?? '';
  }

  void clearSavedAccountSelection() {
    selectedSavedAccount.value = null;
    useSavedAccount.value = false;
