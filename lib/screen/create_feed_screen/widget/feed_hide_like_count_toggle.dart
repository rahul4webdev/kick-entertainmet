...
class FeedHideLikeCountToggle extends StatelessWidget {
  const FeedHideLikeCountToggle({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Image.asset(AssetRes.icHeart, height: 22, width: 22, color: textDarkGrey(context)),
          const SizedBox(width: 12),
          Expanded(
            child: Text(
              LKey.hideLikeCount.tr,
              style: TextStyleCustom.outFitLight300(fontSize: 15, color: textDarkGrey(context)),
            ),
          ),
          CustomToggle(
              isOn: controller.hideLikeCount,
              onChanged: (value) {
                controller.commentHelper.detectableTextFocusNode.unfocus();
                controller.hideLikeCount.value = value;
              })
        ],
      ),
    );
