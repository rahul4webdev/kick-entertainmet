    ...
    return Obx(() {
      ...
      return Column(
        ...
        children: [
          ...
          // Green screen button (hidden during recording)
          if (!shouldStartRecording && controller.isCameraEffects)
            GestureDetector(
              onTap: controller.onGreenScreenTap,
              child: Obx(() => Container(
                width: 38,
                height: 38,
                decoration: BoxDecoration(
                  color: controller.isGreenScreenEnabled.value
                      ? Colors.green.withValues(alpha: 0.7)
                      : Colors.black.withValues(alpha: 0.4),
                  shape: BoxShape.circle,
                ),
                child: const Center(
                  child: Icon(Icons.wallpaper, color: Colors.white, size: 20),
                ),
              )),
            ),

          // Countdown timer button (hidden during recording)
          if (!shouldStartRecording)
            GestureDetector(
              onTap: controller.cycleCountdownSetting,
              child: Container(
                width: 38,
                height: 38,
                decoration: BoxDecoration(
                ...
                child: Center(
                  child: Obx(() {
                    final countdown = controller.countdownSetting.value;
                    return countdown == 0
          ...
          // Ghost/Align toggle
          GestureDetector(
              onTap: () => controller.ghostEnabled.toggle(),
              child: Container(
                width: 38,
                height: 38,
                decoration: BoxDecoration(
                  color: controller.ghostEnabled.value
                ...
                child: const Center(
                  child:
                      Icon(Icons.layers_outlined, color: Colors.white, size: 20),
                ),
              ),
            ),
        ],
