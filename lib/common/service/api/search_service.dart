...
class SearchService {
  ...
  /// Full-text search using PostgreSQL tsvector (ranked by relevance)
  Future<List<Post>> searchPostsFTS({
    String? keyword,
    String? type,
    int? contentType,
    String? genre,
    ...
    PostsModel response = await ApiService.instance.call(
        url: WebService.search.searchPostsFTS,
        param: ({
          Params.limit: AppRes.paginationLimit,
          if ((keyword ?? '').isNotEmpty) Params.keyword: keyword,
          if (type != null) Params.types: type,
